...
const validateUser = [
  ...
  body("emergencyContactPhone")
    ...
    .matches(/^09\d{9}$/)
    .withMessage(
      "Emergency contact phone must be exactly 11 digits starting with 09"
    ),
  ...
  // Section code validation for students
  body("sectionCode")
    .if(body("role").equals("STUDENT"))
    .notEmpty()
    .withMessage("Section code is required for students")
...
const createUserController = [
  ...
  async (req, res) => {
    ...
    try {
      // Verify section code for students
      let sectionId = null;
        ...
        if (!section) {
          return res.status(400).json({
            ...
            errors: [
              {
                path: "sectionCode",
                msg: "Invalid section code. Please check and try again.",
              },
            ],
          });
        }
        if (!section.isActive) {
          return res.status(400).json({
            ...
            errors: [
              {
                path: "sectionCode",
                msg: "This section is not currently active.",
              },
            ],
          });
        }
